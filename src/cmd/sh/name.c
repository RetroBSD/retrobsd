...
struct namnod ps2nod = { (struct namnod *)NIL, &acctnod, ps2name };
struct namnod cdpnod = { (struct namnod *)NIL, (struct namnod *)NIL, cdpname };
struct namnod pathnod = { &mailpnod, (struct namnod *)NIL, pathname };
struct namnod ifsnod = { &homenod, &mailnod, ifsname };
struct namnod ps1nod = { &pathnod, &ps2nod, ps1name };
struct namnod homenod = { &cdpnod, (struct namnod *)NIL, homename };
struct namnod mailnod = { (struct namnod *)NIL, (struct namnod *)NIL, mailname };
struct namnod mchknod = { &ifsnod, &ps1nod, mchkname };
struct namnod acctnod = { (struct namnod *)NIL, (struct namnod *)NIL, acctname };
struct namnod mailpnod = { (struct namnod *)NIL, (struct namnod *)NIL, mailpname };
...
void setlist(arg, xp) register struct argnod *arg;
int xp;
{
    if (flags & exportflg)
        xp |= N_EXPORT;
...
void replace(a, v) register char **a;
char *v;
{
    free(*a);
    *a = make(v);
...
void dfault(n, v) struct namnod *n;
char *v;
{
    if (n->namval == NIL)
        assign(n, v);
...
void assign(n, v) struct namnod *n;
char *v;
{
...
void namscan(fn) void (*fn)(struct namnod *);
{
    namfn = fn;
    namwalk(namep);
}
...
void printro(n) register struct namnod *n;
{
    if (n->namflg & N_RDONLY) {
        prs_buff(readonly);
        prc_buff(SP);
...
void printexp(n) register struct namnod *n;
{
...
void unset_name(name) register char *name;
{
    register struct namnod *n;
    ...
    if ((n = findnam(name))) {
        ...
        if (n == &pathnod || n == &ifsnod || n == &ps1nod || n == &ps2nod || n == &mchknod) {
