...
static void readerr(FILE *fp, char *progname, char *filename)
{
...
static long tzdecode(char *codep)
{
    register int i;
...
int main(int argc, char *argv[])
{
    ...
    ** VERY approximate.
    */
    cuttime = (long)(cutyear - EPOCH_YEAR) * SECS_PER_HOUR * HOURS_PER_DAY * DAYS_PER_NYEAR;
    (void)time(&now);
    longest = 0;
    ...
    for (i = optind; i < argc; ++i) {
        ...
        if (argv[i][0] == '/')
            ...
            if (j > sizeof buf) {
                (void)fprintf(stderr, "%s: timezone name %s/%s is too long\n", argv[0], TZDIR,
                              argv[i]);
                exit(1);
            }
    ...
    if (fflush(stdout) || ferror(stdout)) {
        (void)fprintf(stderr, "%s: Error writing standard output ", argv[0]);
