...
void fatal(int status, char *fmt, char *a)
{
    fflag = 0;
    (void)error(fmt, a);
    exit(status);
...
int chownr(char *dir, int uid, int gid, int savedir)
{
    register DIR *dirp;
    ...
    for (dp = readdir(dirp); dp != NULL; dp = readdir(dirp)) {
        ...
        if (chown(dp->d_name, uid, gid) < 0 && (ecode = Perror(dp->d_name)))
            break;
    }
    if (fchdir(savedir) < 0)
        fatal(255, fchdirmsg, "");
