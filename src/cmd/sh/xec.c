    ...
    if ((t = argt) && execbrk == 0) {
                ...
                if (comtype == NOTFOUND) {
                    ...
                    case SYSCD:
                        if (flags & rshflg)
                            ...
                            if ((cdpath = cdpnod.namval) == NIL || *a1 == '/' || cf(a1, ".") == 0 ||
                                cf(a1, "..") == 0 ||
                                (*a1 == '.' && (a1[1] == '/' || a1[1] == '.' && a1[2] == '/')))
                                cdpath = nullstr;
                            ...
                            if (f)
                                ...
                                cwd(curstak());
                                if (cf(nullstr, dir) && *dir != ':' && any('/', curstak()) &&
                                    flags & prompt) {
                                    prs_buff(curstak());
                                    prc_buff(NL);
                                }
        ...
        case TFORK:
            ...
            } else /* this is the forked branch (child) of execute */
            {
                ...
                if (type != TCOM) {
                    execute(forkptr(t)->forktre, exec_link | 01, errorflg, NULL, NULL);
        ...
        case TLST:
            execute(lstptr(t)->lstlef, 0, errorflg, NULL, NULL);
            execute(lstptr(t)->lstrit, exec_link, errorflg, NULL, NULL);
            break;

        case TAND:
            if (execute(lstptr(t)->lstlef, 0, 0, NULL, NULL) == 0)
                execute(lstptr(t)->lstrit, exec_link, errorflg, NULL, NULL);
            break;
        ...
        case TFOR: {
            ...
            while (*args != ENDARGS && execbrk == 0) {
                assign(n, *args++);
                execute(forptr(t)->fortre, 0, errorflg, NULL, NULL);
                if (breakcnt < 0)
                    execbrk = (++breakcnt != 0);
            }
            if (breakcnt > 0)
        ...
        case TUN: {
            ...
            loopcnt++;
            while (execbrk == 0 &&
                   (execute(whptr(t)->whtre, 0, 0, NULL, NULL) == 0) == (type == TWH)) {
                i = execute(whptr(t)->dotre, 0, errorflg, NULL, NULL);
                if (breakcnt < 0)
                    execbrk = (++breakcnt != 0);
            }
            if (breakcnt > 0)
        ...
        case TIF:
            if (execute(ifptr(t)->iftre, 0, 0, NULL, NULL) == 0)
                execute(ifptr(t)->thtre, exec_link, errorflg, NULL, NULL);
            else if (ifptr(t)->eltre)
                execute(ifptr(t)->eltre, exec_link, errorflg, NULL, NULL);
            else
            ...
            while (regp) {
                ...
                while (rex) {
                    ...
                    if (gmatch(r, s = macro(rex->argval)) || (trim(s), eq(r, s))) {
                        execute(regp->regcom, 0, errorflg, NULL, NULL);
                        regp = NIL;
                        break;
                    } else
                        rex = rex->argnxt;
