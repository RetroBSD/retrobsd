...
int expand(char *as, int rcnt)
{
    ...
    if ((dirf = open(*s ? s : ".", 0)) > 0) {
        if (fstat(dirf, &statb) != -1 && (statb.st_mode & S_IFMT) == S_IFDIR)
            dir++;
        else
            close(dirf);
    }
...
struct direct *getdir(int dirf)
{
    ...
    for (;;) {
        if (dirbuf.dd_loc == 0) {
            dirbuf.dd_size = read(dirf, dirbuf.dd_buf, DIRBLKSIZ);
            if (dirbuf.dd_size <= 0)
                return NULL;
        }
        ...
        if (dp->d_reclen <= 0 || dp->d_reclen > DIRBLKSIZ + 1 - dirbuf.dd_loc)
            return NULL;
