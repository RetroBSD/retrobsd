...
#define MKSHORT(v, lv)                              \
    {                                               \
        U.l = 1L;                                   \
        if (U.c[0])                                 \
            U.l = lv, v[0] = U.s[1], v[1] = U.s[0]; \
        else                                        \
            U.l = lv, v[0] = U.s[0], v[1] = U.s[1]; \
    }
...
int doex(int com)
{
    ...
    ccode = np = 0;
    while ((na = Argv[com++])) {
        if (np >= sizeof nargv / sizeof *nargv - 1)
            break;
        if (strcmp(na, ";") == 0)
            break;
        if (strcmp(na, "{}") == 0)
...
int descend(char *name, char *fname, struct anode *exlist)
{
    ...
    (*exlist->F)(exlist);
    if ((Statb.st_mode & S_IFMT) != S_IFDIR || !Xdev && Devstat.st_dev != Statb.st_dev)
        return 1;
...
int gmatch(char *s, char *p) /* string match as in glob */
{
    if (*s == '.' && *p != '.')
        return 0;
    return amatch(s, p);
...
int list(char *file, struct stat *stp)
{
    ...
    printf("%5u %4ld %s %2d %s%s%s %s %s%s%s\n", stp->st_ino, /* inode #	*/
           ...
           file,          /* name		*/
#ifdef S_IFLNK
           (pmode[0] == 'l') ? " -> " : "", (pmode[0] == 'l') ? flink : "" /* symlink	*/
#else
           "", ""
#endif
    );
